...
class PFENet(nn.Module):
    def __init__(self, layers=50, classes=2, zoom_factor=8, \
        ...
        self.conv_cls = nn.Sequential(
            nn.Conv2d(reduce_dim, reduce_dim//2, kernel_size=5,stride=2, padding=1, bias=False),
            nn.ReLU(inplace=True),   
            nn.Conv2d(reduce_dim//2, reduce_dim//4, kernel_size=5, stride=2, padding=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(reduce_dim//4, reduce_dim//8, kernel_size=5, stride=2, padding=1, bias=False),
            nn.ReLU(inplace=True)                             
        )

        # self.res101_conv_cls = nn.Sequential(
        #     nn.Conv2d(reduce_dim, reduce_dim//2, kernel_size=5,stride=2, padding=1, bias=False),
        #     nn.ReLU(inplace=True),   
        #     nn.Conv2d(reduce_dim//2, reduce_dim//4, kernel_size=5, stride=2, padding=1, bias=False),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(reduce_dim//4, reduce_dim//4, kernel_size=5, stride=2, padding=1, bias=False),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(reduce_dim//4, reduce_dim//8, kernel_size=5, stride=2, padding=1, bias=False),
        #     nn.ReLU(inplace=True)                             
        # )

        #self.mlp_pascal = MLP(1152, 21, bias=True)  #res50 conv_cls
        #self.mlp_pascal = MLP(128, 20, bias=True)#vgg16 conv_cls//res50 101conv
        #self.mlp_pascal = MLP(128, 15, bias=True) #res50 101conv
        self.mlp_pascal = MLP(1152, 15, bias=True) #res50 conv_cls
        #self.mlp_coco = MLP(512, 60, bias=True)    #vgg16             
        #self.mlp_coco = MLP(2592, 60, bias=True) #res101  #res50
        #self.mlp_coco = MLP(512, 60, bias=True)
    ...
    def forward(self, subcls,x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), y=None):
        x_size = x.size()
        ...
        query_conv = self.conv_cls(query_feat) #[4,32,6,6]#[4,32,2,2]#[8,32,9,9]
        #query_conv = self.res101_conv_cls(query_feat)
        bc = query_conv.size(0)
        q_cls = query_conv.view(bc,-1) #[4,128]
        #q_cls = self.mlp_coco(q_cls) #[4,21]
        q_cls = self.mlp_pascal(q_cls) #[4,21]
        q_loss = self.criterion(q_cls, subcls.long())
        ...
        for i in range(self.shot):
            ...
            # supp_feat_c = supp_feat * mask #[4,256,60,60]
            # #supp_conv = self.res101_conv_cls(supp_feat_c) #[4,32,6,6]
            # supp_conv = self.conv_cls(supp_feat_c)
            # bc = supp_conv.size(0)
            # s_cls = supp_conv.view(bc,-1)
            # #s_cls = self.mlp_coco(s_cls) #[4,21]
            # s_cls = self.mlp_pascal(s_cls)
            # s_loss = self.criterion(s_cls, subcls.long())
            # s_loss_list.append(s_loss)


            #supp_conv = self.res101_conv_cls(supp_feat_c) #[4,32,6,6]
            supp_conv = self.conv_cls(supp_feat)
            bc = supp_conv.size(0)
            s_cls = supp_conv.view(bc,-1)
            #s_cls = self.mlp_coco(s_cls) #[4,21]
            s_cls = self.mlp_pascal(s_cls)
        ...
        # if a == 9:
        #     pdb.set_trace()
        #     res = corr_query_mask.sigmoid().data.cpu().numpy().squeeze()
        #     res = (res - res.min()) / (res.max() - res.min() + 1e-8)
        #     save_path = './images/prior/'
        #     print('save img to: ',save_path+'9.png')
        #     cv2.imwrite(save_path+'9.png',res*255)
        ...
        if self.shot > 1:
            ...
            s_loss = s_loss_list[0]
            for i in range(1, len(s_loss_list)):
                s_loss += s_loss_list[i]
            s_loss /= len(s_loss_list)
        ...
        if self.training:
            ...
            return out.max(1)[1], main_loss, aux_loss, s_loss, q_loss
        else:
            return out
