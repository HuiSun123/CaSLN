...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    optimizer = torch.optim.SGD(
        ...
        {'params': model.mlp_pascal.parameters()},
        #{'params': model.res101_conv_cls.parameters()},
        {'params': model.conv_cls.parameters()},       
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss, s_loss, q_loss = model(subcls[0], s_x=s_input, s_y=s_mask, x=input, y=target)

        if not args.multiprocessing_distributed:
            main_loss, aux_loss, s_loss, q_loss = torch.mean(main_loss), torch.mean(aux_loss), torch.mean(s_loss), torch.mean(q_loss)
        loss = main_loss + args.aux_weight * aux_loss + 0.01 * s_loss + 0.003 * q_loss
        #loss = main_loss + args.aux_weight * aux_loss + s_loss + q_loss
        optimizer.zero_grad()
        ...
        n = input.size(0)
        if args.multiprocessing_distributed:
            main_loss, aux_loss, s_loss, q_loss, loss = main_loss.detach() * n, aux_loss * n, s_loss * n, q_loss * n, loss * n 
            count = target.new_tensor([n], dtype=torch.long)
            dist.all_reduce(main_loss), dist.all_reduce(aux_loss), dist.all_reduce(s_loss), dist.all_reduce(q_loss), dist.all_reduce(loss), dist.all_reduce(count)
            n = count.item()
            main_loss, aux_loss, s_loss, q_loss, loss = main_loss / n, aux_loss / n, s_loss / n, q_loss / n, loss / n
        ...
        s_loss_meter.update(s_loss.item(), n)
        q_loss_meter.update(q_loss.item(), n)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'SLoss {s_loss_meter.val:.4f} '
                        'QLoss {q_loss_meter.val:.4f} '                   
                        ...
                        'Accuracy {accuracy:.4f}.'.format(epoch+1, args.epochs, i + 1, len(train_loader),
                                                          ...
                                                          s_loss_meter=s_loss_meter,
                                                          q_loss_meter=q_loss_meter,
...
def validate(val_loader, model, criterion):
    ...
    for e in range(10):
        for i, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            output = model(subcls[0], s_x=s_input, s_y=s_mask, x=input, y=target)
