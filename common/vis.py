...
class Visualizer:
    ...
    def initialize(cls, visualize = "True"):
        cls.visualize = visualize
        if not visualize:
            return

        cls.colors = {'red': (255, 50, 50), 'blue': (0, 0, 255), 'green': (50, 255, 50)}
    ...
    def visualize_prediction_batch(cls, spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b, batch_idx, iou_b=None):
        ...
        spt_img = torch.tensor(spt_img_b[0]).cuda().data.cpu().numpy()
        spt_mask = torch.tensor(spt_mask_b[0]).cuda().data.cpu().numpy()
        qry_img = torch.tensor(qry_img_b[0]).cuda().data.cpu().numpy()
        qry_mask = torch.tensor(qry_mask_b[0]).cuda().data.cpu().numpy()
        pred_mask = torch.tensor(pred_mask_b[0]).cuda().data.cpu().numpy()
        #pdb.set_trace()
        cls_id = cls_id_b
